...
Get-Content -Path "c:\Users\usr\Documents\Advent Of Code\2022\Day 5\SupplyStacks.txt" | ForEach-Object{
    ...
    $upperidx = $allstacks.Item($procedure[1] - 1).Count
    $loweridx = $allstacks.Item($procedure[1] - 1).Count - $procedure[0]
    for($idx = $loweridx; $idx -lt $upperidx; $idx++)
    {
        $allstacks.Item($procedure[2]-1).Add($allstacks[$procedure[1]-1][$idx])
    }
    $allstacks.Item($procedure[1]-1).RemoveRange($loweridx, $procedure[0])
}
